@echo off
REM Build a self-contained OspreySharp executable for bundling with the Carafe
REM Windows distribution. OspreySharp is the MacCoss-lab DIA search tool in the
...
REM and stages OspreySharp.exe under <output-root>\win-x64\ so Carafe's
...
REM   OSPREY_CSPROJ  Path to OspreySharp.csproj
...
if "%OSPREY_CSPROJ%"=="" set "OSPREY_CSPROJ=%USERPROFILE%\GitHub-Repo\ProteoWizard\pwiz\pwiz_tools\OspreySharp\OspreySharp\OspreySharp.csproj"
...
if not exist "%OSPREY_CSPROJ%" (
    echo ERROR: OspreySharp.csproj not found at: %OSPREY_CSPROJ% 1>&2
...
echo.
echo Staged OspreySharp build under %DEST%.
