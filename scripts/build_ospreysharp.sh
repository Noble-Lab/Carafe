#!/usr/bin/env bash
#
# Build self-contained OspreySharp executables for bundling with the Carafe
# distribution. OspreySharp is the MacCoss-lab DIA search tool that lives in the
...
# resolveOspreyBinary() can locate OspreySharp(.exe) next to the jar (or under
...
#   OSPREY_CSPROJ  Path to OspreySharp.csproj
#                  (default: ~/GitHub-Repo/ProteoWizard/pwiz/pwiz_tools/OspreySharp/OspreySharp/OspreySharp.csproj)
...
OSPREY_CSPROJ="${OSPREY_CSPROJ:-${HOME}/GitHub-Repo/ProteoWizard/pwiz/pwiz_tools/OspreySharp/OspreySharp/OspreySharp.csproj}"
...
if [[ ! -f "${OSPREY_CSPROJ}" ]]; then
    echo "ERROR: OspreySharp.csproj not found at: ${OSPREY_CSPROJ}" >&2
...
echo "OspreySharp csproj : ${OSPREY_CSPROJ}"
...
for rid in "${RIDS[@]}"; do
    ...
    if [[ "${rid}" != win-* ]]; then
        if [[ -f "${dest}/OspreySharp" ]]; then
            chmod +x "${dest}/OspreySharp"
        fi
    fi
    echo "    done: ${dest}"
done
...
echo
echo "Staged OspreySharp builds under ${OUTPUT_ROOT}."
