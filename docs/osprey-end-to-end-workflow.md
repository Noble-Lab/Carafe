# The Osprey end-to-end workflow

This document describes, step by step, what Carafe2 does when it runs a spectral-library
workflow with **Osprey** as the search engine — from the input protein FASTA all the way
...
> **Background.** Unlike DIA-NN, Osprey does not build its own AI-predicted library.
> Carafe2 therefore has to *supply* Osprey with a library (targets **and** decoys), let
> Osprey search the training runs against it, and then fine-tune Carafe2's RT / fragment
> intensity / ion-mobility models on Osprey's identifications. The fine-tuned models are
> what generate the final, experiment-specific library.

---

## Which workflows use Osprey

Two entries in the **Workflow** dropdown drive Osprey:

| # | Workflow | What it does |
|---|----------|--------------|
| **4** | Osprey: search, finetune, build new library | Searches the **training** runs, fine-tunes the models, builds a new fine-tuned library. **Stops there.** |
| **5** | Osprey: end-to-end (finetune, then search project files) | Everything Workflow 4 does, **plus** a second Osprey search of your **project** runs using the new fine-tuned library. |
...
```
 ...
 (sequential)   Stage 2  Build INITIAL library  ◀──────────┘
                Stage 3  Osprey search of TRAIN runs  →  osprey.blib
                Stage 4  Fine-tune RT / MS2 / CCS models on osprey.blib + mzML
                Stage 5  Build FINAL (fine-tuned) library
                Stage 6  (Workflow 5 only) Osprey search of PROJECT runs
```
...
of concurrent MSConvert processes is set on the Osprey tab (default 4).

---

### Stage 0 — MSConvert: raw files → mzML

Osprey reads **mzML**. Any training or project input that is not already mzML (Thermo
...
- The MSConvert executable is taken from the Osprey tab (or the saved preference, or
...
This is the step that makes Osprey's library. Carafe2 digests the protein FASTA into
...
(targets and decoys alike), producing the library Osprey will search against. There are two
predictor options, chosen on the Osprey tab:
...
### Stage 3 — Osprey search of the training runs

Osprey searches the converted training mzMLs against the initial library. The key part is
that the library already contains decoys, and the pairing manifest tells Osprey how targets
and decoys correspond — see [Decoys in the library](#decoys-in-the-library-and-the-pairing-manifest).

The command is shaped like:

```
Osprey -i <run1.mzML> <run2.mzML> ... \
...
- FDR is always estimated with **Percolator**; the run / experiment / protein FDR thresholds and
  the FDR level come from the Osprey tab.
...
2. **Extract measured fragments.** From the **training mzMLs**, Carafe2 extracts the *observed*
   ...
   as it does for a DIA-NN report. (Osprey gives the IDs; the mzML gives the empirical
...
### Stage 6 — Osprey search of the project runs *(Workflow 5 only)*

In the end-to-end workflow, Carafe2 takes one more step: it runs Osprey again, this time
searching your **project** mzMLs against the **fine-tuned** library from Stage 5 (the same
`Osprey -i … -l … -o … --decoys-in-library --decoy-pairing-manifest …` shape as Stage 3,
...
| **Osprey** | An argument list run with `ProcessBuilder`; binary located by `resolveOspreyBinary()`. | `buildOspreyCommand` |
...
once. The first task to exit non-zero aborts the run. The Osprey workflow uses two phases:
...
- **Phase 2 (one sequential lane):** the deferred local/Koina initial library → Osprey train
  search → fine-tune + new library → (Workflow 5) Osprey project search.
...
The executable is resolved from the Osprey-tab combo → the saved preference → `msconvert` on
...
**Osprey** (`buildOspreyCommand`) — an argument list. The binary is found by
`resolveOspreyBinary()` in order: (1) saved path preference / Osprey-tab combo, (2) bundled
`<jarDir>/osprey/<rid>/Osprey(.exe)`, (3) `~/.carafe/osprey/<rid>/…`, (4) `where`/`which` on
`PATH`, where `<rid>` is `win-x64` / `osx-arm64` / `osx-x64` / `linux-x64`. Field → flag:

| GUI field | Osprey flag |
|-----------|------------------|
...
| Osprey additional-options field | appended verbatim |
...
the DIA-NN workflows use. Rather than write a second command builder, the Osprey path
...
Osprey expects a library that **already contains decoys**, with retention times and spectra
...
- **`--decoys-in-library`** — tells Osprey that decoy entries are present in the `-l` library
  itself (rather than asking Osprey to generate decoys on the fly). FDR is then estimated
  against those library decoys.
- **`--decoy-pairing-manifest pairing.tsv`** — points Osprey at the manifest so it knows
...
(target + p_target + decoy + p_decoy). The index is what lets Osprey pair a decoy back to its
...
consistency is all Osprey requires.
...
A typical Osprey run (Workflow 5) produces:
...
|-----------|---------|---------|
| `-se Osprey` | search/fine-tune | Select Osprey as the search engine. |
...
| `--decoys-in-library`, `--decoy-pairing-manifest` | Stage 3 / 6 | Tell Osprey the library contains decoys and how they pair. |
| `-i <blib>` | Stage 4 | Fine-tune on an Osprey `.blib` (in place of a DIA-NN report). |
...
- `README.md` → **"Using Osprey as the search engine"** for the GUI walkthrough and
  Osprey installation/bundling details.
- `scripts/build_osprey.sh` / `.bat` for building the self-contained Osprey binary.
